...
// Igual ao writeEncryptedCookies, mas guarda também a URL onde a tela de 2FA
// apareceu, pra o verify2fa voltar exatamente pra ela (e não pra /Account/Login,
// que só mostra o formulário de usuário/senha de novo).
function writeEncryptedPending(res, name, altenarCookies, url, maxAgeSeconds) {
  const payload = { cookies: altenarCookies, url: url || null, expiresAt: Date.now() + maxAgeSeconds * 1000 };
  const token = encrypt(payload);
  const existing = res.getHeader('Set-Cookie');
  const newCookie = `${name}=${token}; Path=/; HttpOnly; Secure; SameSite=Strict; Max-Age=${maxAgeSeconds}`;
  const all = existing ? (Array.isArray(existing) ? existing.concat(newCookie) : [existing, newCookie]) : [newCookie];
...
function readEncryptedPending(req, name) {
  const cookies = parseCookies(req);
  const token = cookies[name];
  if (!token) return null;
  try {
    ...
    if (!data || !Array.isArray(data.cookies)) return null;
    if (data.expiresAt && Date.now() > data.expiresAt) return null;
    return { cookies: data.cookies, url: data.url || null };
  } catch (e) {
    return null;
  }
}
...
module.exports = {
  ...
  // estado intermediário entre senha e código 2FA (guarda cookies + a URL da tela de código)
  getPending: (req) => readEncryptedPending(req, PENDING_COOKIE),
  setPendingCookies: (res, cookies, url) => writeEncryptedPending(res, PENDING_COOKIE, cookies, url, PENDING_MAX_AGE),
